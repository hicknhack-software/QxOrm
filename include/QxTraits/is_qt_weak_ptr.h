 ...
 */
template <typename T>
class is_qt_weak_ptr
{

private:

   template <typename V> static char isSmartPtr(QWeakPointer<V>);
   static int isSmartPtr(...);
   static T t;

public:

   enum { value = (sizeof(qx::trait::is_qt_weak_ptr<T>::isSmartPtr(t)) == sizeof(char)) };

   typedef typename boost::mpl::if_c<qx::trait::is_qt_weak_ptr<T>::value, 
                                     boost::mpl::true_, 
